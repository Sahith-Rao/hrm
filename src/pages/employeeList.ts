...
export class EmployeeListPage {
  ...
  async searchByEmployeeId(empId: string) {
    ...
    if (await spinner.isVisible().catch(() => false)) {
      await expect(spinner).toBeHidden({ timeout: 1500000 });
    }
    await this.page.waitForLoadState("networkidle");
    const noRecords = this.page.getByText("No Records Found", { exact: true }).first();
    await Promise.race([
      this.#tableBody.waitFor({ state: "visible", timeout: 1500000 }),
      noRecords.waitFor({ state: "visible", timeout: 1500000 }),
    ]);
  ...
  async verifyEmployeePresent(empId: string) {
    ...
    await expect(row.first()).toBeVisible({ timeout: 1500000 });
  ...
  async clickEditByEmployeeId(empId: string) {
    ...
    await expect(editIcon).toBeVisible({ timeout: 1500000 });
  ...
  async verifyFirstNameInTable(empId: string, expectedFirstName: string) {
    const row = this.#rowByEmployeeId(empId);
    await expect(row.first()).toBeVisible({ timeout: 1500000 });
    await expect(row.first()).toContainText(expectedFirstName, { timeout: 1500000 });
  ...
  async deleteEmployeeById(empId: string) {
    ...
    await expect(deleteIcon).toBeVisible({ timeout: 1500000 });
    await deleteIcon.click();
    console.log("Clicked Delete icon");
    const confirmDeleteBtn = this.page.getByRole("button", { name: /Yes, Delete/i });
    await expect(confirmDeleteBtn).toBeVisible({ timeout: 1500000 });
    await confirmDeleteBtn.click();
    console.log("Confirmed delete");
    await expect(row).toHaveCount(0, { timeout: 1500000 });
